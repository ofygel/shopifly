...
interface Props {
  items: Product[]
  viewMode?: 'grid' | 'list'  // Добавим опциональный проп для удобства, по умолчанию 'grid'
}

const Carousel: React.FC<Props> = ({ items, viewMode = 'grid' }) => {
  ...
  return (
    <div className="relative w-full">
      ...
      <div ref={ref} className="flex gap-6 overflow-x-auto scroll-snap-x snap-mandatory px-12 scrollbar-hide">
        {items.map(p => (
          <div key={p.id} className="snap-start flex-shrink-0 w-72">
